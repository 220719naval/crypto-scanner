...
def http_get(url, params=None, timeout=12, retries=2):
    last_error = None
...
def get_klines_cached(symbol, interval, limit=220):
    ...
    # Hard cap prevents the 200-coin scan from accumulating hundreds of
    # pandas DataFrames and getting SIGKILLed by PythonAnywhere resource limits.
    if len(CANDLE_CACHE) > CANDLE_CACHE_MAX_ITEMS:
        oldest_key = next(iter(CANDLE_CACHE))
        if oldest_key != key:
            CANDLE_CACHE.pop(oldest_key, None)
        else:
            # Keep the just-fetched frame; evict the next oldest entry.
            keys = iter(CANDLE_CACHE)
            next(keys, None)
            victim = next(keys, None)
            if victim is not None:
                CANDLE_CACHE.pop(victim, None)
    return df
...
def _fast_stage_candidate(df4):
    """Broad prefilter only; never replaces the 72-rule engine."""
    try:
        if df4 is None or len(df4) < 80:
            return False
        c = df4["close"]
        v = df4["volume"]
        e20 = ema(c, 20).iloc[-1]
        e50 = ema(c, 50).iloc[-1]
        rr = rsi(c, 14).iloc[-1]
        recent_high = df4["high"].iloc[-21:-1].max()
        vol_ma = v.rolling(20).mean().iloc[-1]
        vr = (v.iloc[-1] / vol_ma) if vol_ma and np.isfinite(vol_ma) else 0.0
        close = float(c.iloc[-1])
        # Keep the prefilter deliberately broad: either trend/momentum,
        # breakout proximity, or a possible bearish structure can proceed.
        bull = close >= e20 * 0.985 and e20 >= e50 * 0.985 and rr >= 44
        bear = close <= e20 * 1.015 and e20 <= e50 * 1.015 and rr <= 56
        breakout = close >= recent_high * 0.985 or vr >= 1.35
        return bool(bull or bear or breakout)
    except Exception:
        return True
...
def run_scan(exchange_symbols, tickers, cg, usdinr, fixed_symbols=None):
    ...
    # ================================================================
    # STAGE 1: lightweight 4H prefilter in a small bounded thread pool.
    # Only 4H frames are held here; this avoids the old 200 x 4-TF
    # memory buildup that could trigger PythonAnywhere SIGKILL.
    # ================================================================
    stage1 = {}

    def _stage1_one(item):
        idx, symbol = item
        try:
            ticker = tickers.get(symbol, {})
            passed, qscore, qflags = quality_gate(symbol, ticker, cgmap)
            if not passed:
                return idx, symbol, "quality", None, qscore, qflags, None
            df4 = get_klines_cached(symbol, "4h", 230)
            if not _fast_stage_candidate(df4):
                return idx, symbol, "fast", None, qscore, qflags, None
            return idx, symbol, "candidate", df4, qscore, qflags, None
        except ValueError as e:
            if "INSUFFICIENT_HISTORY" in str(e) or "insufficient" in str(e).lower() or "not enough closed" in str(e).lower():
                return idx, symbol, "history", None, 0, [], str(e)
            return idx, symbol, "error", None, 0, [], repr(e)
        except Exception as e:
            return idx, symbol, "error", None, 0, [], repr(e)

    # Keep concurrency deliberately small for PythonAnywhere.
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = [pool.submit(_stage1_one, item) for item in enumerate(symbols, 1)]
        for fut in as_completed(futures):
            idx, symbol, status, df4, qscore, qflags, err = fut.result()
            stage1[idx] = (symbol, status, df4, qscore, qflags, err)

    # Stage 2 remains sequential: only candidates get the expensive 1H/1D/1W
    # data and the complete 72-rule + expert evaluation.
    for idx in range(1, len(symbols) + 1):
        symbol, status, df4, qscore, qflags, err = stage1[idx]
        try:
            if status == "quality":
                print(f"[{idx}/{len(symbols)}] {symbol}: QUALITY BLOCK {qflags}")
                continue
            if status == "fast":
                technical_rejects += 1
                print(f"[{idx}/{len(symbols)}] {symbol}: FAST PREFILTER")
                continue
            if status == "history":
                history_skips += 1
            ...
            if status == "error":
                errors += 1
                print(f"[{idx}/{len(symbols)}] {symbol} ERROR: {err}")
                continue

            dfs = {"4h": df4}
            dfs["1h"] = get_klines_cached(symbol, "1h", 230)
            dfs["1d"] = get_klines_cached(symbol, "1d", 230)
            dfs["1w"] = get_klines_cached(symbol, "1w", 230)

            latest = dfs["1h"]["close_time"].iloc[-1]
            age_min = max(0.0, (pd.Timestamp.now(tz="UTC") - latest).total_seconds() / 60)
            if age_min > MAX_KLINE_AGE_MIN:
                stale_skips += 1
                print(f"[{idx}/{len(symbols)}] {symbol}: STALE DATA ({age_min:.1f}m)")
                continue
            ...
            r = evaluate(symbol, dfs, funding, oi, btc, qscore, qflags)
            if r:
                results.append(r)
                print(f"🟢 {symbol} {r['direction']} {r['score']}/72 ({r['percentage']:.1f}%) Q={r['quality_score']:.0f}")
            else:
                technical_rejects += 1
                print(f"[{idx}/{len(symbols)}] {symbol}: no qualified setup")
            ...
            errors += 1
            print(f"[{idx}/{len(symbols)}] {symbol} ERROR TYPE: {type(e).__name__}")
            print(f"[{idx}/{len(symbols)}] {symbol} ERROR DETAIL: {repr(e)}")
        finally:
            # Critical memory fix: do not retain four pandas DataFrames for every
            # symbol. The previous code only evicted 1H, allowing 4H/1D/1W to
            # accumulate throughout the 200-coin scan.
    ...
    # Release all Stage-1 references before the next scan cycle.
    stage1.clear()
    gc.collect()
